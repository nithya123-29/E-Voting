...
#		end = begin + ipHeaderLength
	...
	if printKey == 0:
		...
		if (extractedAttIndex == 14) or (extractedAttIndex == 0):
			unpackedInfo.append('optional Address: ' + str(ipopt))
	else:
		if (extractedAttIndex == 1):
...
def startSniff():
	try:
		while True:
			...
			decision = input('Sniff the network? Y/N: ')
...
def startFilter():
	try:
		while True:
			...
			decision = input('Filter the packets by a specific protocol? Y/N: ')
...
def startExtract():
	try:
		while True:
			...
			decision = input('Extract a specific attribute from the protocol? Y/N: ')
...
def startCalculations():
	try:
		while True:
			...
			decision = input('Would you like to do some calculations? Y/N: ')
...
#	if os == linux:
	...
	while True:
		...
		filteredProtocolIndex = input('0: All\n1: ARP\n2: ICMP\n3: TCP\n4: UDP\nSelection: ')
...
#	if os == linux:
	...
	while True:
		...
		if filteredProtocolIndex == 0:
			...
			print(allAttributes)
			print(ethAttributes)
			extractedAttIndex = input('Selection: ')
...
def main(): #	if (os == linux) or (os == windows):
    while True: # Ask the user to begin sniffing.
	    startSniff()
